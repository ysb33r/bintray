...
class BintrayAPI {
    ...
    HttpResponseDecorator getLatestVersion() {
        final String rest = "packages/${repoOwner}/${repoName}/${packageName}"
        HttpResponseDecorator response
        try {
            response = client().get( path : "${rest}/versions/_latest"  )
    ...
    HttpResponseDecorator getFileListForVersion(String version, Boolean includeUnpublished) {
        final String rest = "packages/${repoOwner}/${repoName}/${packageName}"
        HttpResponseDecorator response
        try {
            response = client().get(
    ...
    HttpResponseDecorator getFileListLatestVersion(String version, Boolean includeUnpublished) {
        HttpResponseDecorator response = getFileListForVersion(getLatestVersion(),includeUnpublished)
        return response
    }

    HttpResponseDecorator fileHasUpdate(String filename) {
        Map fileList = getFileListForVersion(getLatestVersion(),includeUnpublished).data.
        return response
    }
